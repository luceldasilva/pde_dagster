...
from pde_dagster.ops.J3_partitioned_process_records.extract_table import (
    extract_table_op
)
from pde_dagster.ops.J3_partitioned_process_records.transform_record import (
    transform_record_op
)
...
def partitioned_config(start, _end):
    return {
        "ops": {
            "extract_table_op": {
                "config": {
                    "start_time": start.replace(tzinfo=None).strftime("%Y-%m-%d"),
                    "end_time": _end.replace(tzinfo=None).strftime("%Y-%m-%d")
                }
            }
        },
        "resources": {
...
@job(
    ...
    resource_defs={
        ...
        "job_config": make_values_resource(table_name=str)
    },
